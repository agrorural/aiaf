...
add_filter( 'map_meta_cap', 'institucion_meta_cap', 10, 4 );
function institucion_meta_cap( $caps, $cap, $user_id, $args ) {

	/* If editing, deleting, or reading a producto, get the post and post type object. */
	if ( 'edit_institucion' == $cap || 'delete_institucion' == $cap || 'read_institucion' == $cap ) {
		$post = get_post( $args[0] );
		$post_type = get_post_type_object( $post->post_type );
